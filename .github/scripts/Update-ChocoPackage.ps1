...
<#
.SYNOPSIS
    Checks for a new Helix P4 (command-line client) installer build and updates
    the Chocolatey package files.

.DESCRIPTION
    This script is designed to run in a GitHub Actions workflow on a windows-latest runner.
    It discovers the latest Perforce P4 release, compares the SHA256 hash of helix-p4-x64.exe
    against the hash currently in chocolateyinstall.ps1, and if a change is detected, it
    silently installs the new binary, extracts the full version via 'p4.exe -V', and updates
    p4.nuspec and tools/chocolateyinstall.ps1.
...
$installerFileName = 'helix-p4-x64.exe'

$installScriptPath = Join-Path $RepoRoot 'tools\chocolateyinstall.ps1'
$nuspecPath        = Join-Path $RepoRoot 'p4.nuspec'
...
function Get-LatestReleaseDirectory {
    <#
    .SYNOPSIS
        Scrapes the Perforce file host index for release directories and returns them
        sorted newest-first. Then probes each to find the latest one that actually
        hosts helix-p4-x64.exe.
    #>
...
function Get-RemoteChecksum {
    ...
    <#
    .SYNOPSIS
        Downloads the SHA256SUMS file for a release and extracts the hash for helix-p4-x64.exe.
    #>
...
function Install-P4Silently {
    param([string]$InstallerPath)
    <#
    .SYNOPSIS
        Silently installs the Helix P4 command-line client.
    #>
    Write-Log "Installing P4 silently from $InstallerPath ..."
    ...
    Write-Log "P4 installation completed successfully."
    return $true
}

function Get-P4Version {
    <#
    .SYNOPSIS
        Runs 'p4 -V' to extract the full version string and converts it to
        a Chocolatey-compatible version (e.g. 2025.1.2880859).
    .NOTES
        p4 -V outputs something like:
          Rev. P4/NTX64/2025.1/2880859 (2025/03/15).
        We want to extract "2025.1.2880859".
    #>
    # Search common install locations
    $searchPaths = @(
        "${env:ProgramFiles}\Perforce\p4.exe",
        "${env:ProgramFiles(x86)}\Perforce\p4.exe"
    )
    ...
    if (-not $p4Exe) {
        ...
        Write-Log "p4.exe not found in standard locations, searching Program Files..."
        $found = Get-ChildItem -Path $env:ProgramFiles -Filter 'p4.exe' -Recurse -ErrorAction SilentlyContinue | Select-Object -First 1
        if ($found) {
            $p4Exe = $found.FullName
        }
    }

    if (-not $p4Exe) {
        Write-Log "Could not locate p4.exe after installation." -Level ERROR
        return $null
    }

    Write-Log "Found p4.exe at: $p4Exe"
    Write-Log "Running: & `"$p4Exe`" -V"

    # p4 -V prints version info to stdout then exits
    try {
        $output = & $p4Exe -V 2>&1 | Out-String
    }
    catch {
        Write-Log "Failed to run p4.exe -V: $_" -Level ERROR
        return $null
    }

    Write-Log "p4.exe -V output:`n$output"

    # Parse: "Rev. P4/NTX64/2025.1/2880859"  →  "2025.1.2880859"
    $match = $output | Select-String -Pattern 'P4/\w+/(\d{4}\.\d+)/(\d+)'
    if ($match) {
        $majorMinor = $match.Matches[0].Groups[1].Value   # e.g. 2025.1
        $changelist = $match.Matches[0].Groups[2].Value    # e.g. 2880859
        $version    = "$majorMinor.$changelist"             # e.g. 2025.1.2880859
        Write-Log "Extracted version: $version"
        return $version
    }

    Write-Log "Could not parse version from p4.exe -V output." -Level ERROR
    return $null
}
...
function Update-PackageFiles {
    ...
    <#
    .SYNOPSIS
        Updates p4.nuspec and tools/chocolateyinstall.ps1 with the new version and checksum.
    #>
...
Write-Log '════════════════════════════════════════════════════════════════'
Write-Log '  P4 Chocolatey Package Auto-Updater'
Write-Log '════════════════════════════════════════════════════════════════'
...
$tempDir       = Join-Path $env:TEMP "p4-update-$(Get-Date -Format 'yyyyMMddHHmmss')"
...
$installOk = Install-P4Silently -InstallerPath $installerPath
if (-not $installOk) {
    Write-Log "Aborting: P4 installation failed." -Level ERROR
    exit 1
}

$newVersion = Get-P4Version
if (-not $newVersion) {
    Write-Log "Aborting: could not extract version from p4.exe." -Level ERROR
    exit 1
}
